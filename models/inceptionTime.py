...
class Inception(nn.Module):
    def __init__(self, in_channels, n_filters, kernel_sizes=[9, 19, 39], bottleneck_channels=32, activation=nn.ReLU(),
        ...
        Args:
            ...
            kernel_sizes: List of kernel sizes for each convolution. Each kernel size must be odd number that meets ->
                "kernel_size % 2 !=0". This is necessary because of padding size. For correction of kernel_sizes use
                function "correct_sizes".
            bottleneck_channels: Number of output channels in bottleneck. Bottleneck won't be used if number of
                in_channels is equal to 1.
